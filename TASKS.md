...
**Status:** Phase 0 (P0-001..006) and Phase 1 (P1-001..018) are complete, with a Vitest unit + integration baseline in place. **Phase 2 is complete.** Epic 1 — Capture pipeline (P2-001..007): settings, sidecar candidate queue, auto-capture provenance, dedup, LLM extraction, capture-policy gate, and auto-capture orchestration. Epic 2 — Control surface & integration (P2-008..015): the `memory_review`/`memory_revert` tools + `/memory-review`/`/memory-revert` commands, Phase 2 status/maintenance, the `src/llm.ts` model adapter, the extension hook wiring (auto-capture on `session_compact`/`session_tree`, the live-turn nudge), and the skill/prompt/README/testing-strategy updates. All engine logic is unit/integration-tested with a faked `LlmFn` (121 tests); the live wiring is validated in `docs/phase2-validation.md`.
